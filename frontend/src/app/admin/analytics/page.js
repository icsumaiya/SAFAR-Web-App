...
export default function AnalyticsPage() {
  ...
  const statCards = [
    ...
    { label: "Total Revenue", value: `$${Number(summary.total_revenue).toFixed(2)}` },
    { label: "Platform Commission", value: `$${Number(summary.platform_commission).toFixed(2)}` },
    { label: "Agency Earnings", value: `$${Number(summary.agency_earnings).toFixed(2)}` },
  ...
  return (
    <AdminLayout title="Analytics Dashboard">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {statCards.map((c) => (
          <div key={c.label} className="bg-safar-card rounded-xl shadow-sm border border-black/5 p-4 text-center">
        ...
        <ChartCard title="Monthly Bookings">
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={charts.monthly_bookings}>
              ...
              <YAxis fontSize={12} />
              <Tooltip />
              <Line type="monotone" dataKey="total" stroke="#FF7D4B" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Monthly Revenue">
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={charts.monthly_revenue}>
              ...
              <YAxis fontSize={12} />
              <Tooltip />
              <Line type="monotone" dataKey="total" stroke="#009688" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Popular Packages">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={charts.popular_packages} layout="vertical">
              ...
              <YAxis dataKey="title" type="category" width={100} fontSize={11} />
              <Tooltip />
        ...
        <ChartCard title="Popular Destinations">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={charts.popular_destinations} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" fontSize={12} />
              <YAxis dataKey="location" type="category" width={100} fontSize={11} />
              <Tooltip />
        ...
        <ChartCard title="Top Agencies by Revenue">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={charts.top_agencies} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" fontSize={12} />
              <YAxis dataKey="company_name" type="category" width={100} fontSize={11} />
              <Tooltip />
